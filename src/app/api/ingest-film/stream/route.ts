...
export async function POST(request: Request) {
  ...
  const reclassifyShotIds = Array.isArray(body.reclassifyShotIds)
    ? [...new Set(body.reclassifyShotIds.filter((x): x is string => typeof x === "string" && uuidRe.test(x.trim())).map((s) => s.trim()))]
    : [];
  ...
  if (reclassifyShotIds.length > 0) {
    return new Response(
      JSON.stringify({
        error:
          "Selective shot reclassify (reclassifyShotIds) requires the TS ingest worker. Set INGEST_WORKER_URL or NEXT_PUBLIC_WORKER_URL so this route proxies to the worker.",
      }),
